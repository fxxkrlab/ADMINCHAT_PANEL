      ...
      <div className="flex-1 px-8 py-6 overflow-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 rounded-md hover:bg-bg-elevated text-text-secondary hover:text-white transition-colors"
              title="Back"
            >
              <ArrowLeft size={18} />
            </button>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate('/faq')}
              className="px-4 py-2 rounded-lg text-sm text-text-secondary border border-border hover:text-white hover:bg-bg-elevated transition-colors"
            >
              Cancel
            </button>
            <button
        ...
        <div className="flex items-end gap-4 mb-6">
          <div className="flex-1">
            <label className="block text-[13px] font-medium text-text-secondary mb-2 font-['Inter']">Rule Name</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="Rule name..."
              className="w-full h-10 px-3.5 bg-bg-elevated border border-border rounded-lg text-sm text-white placeholder:text-text-placeholder focus:outline-none focus:border-accent transition-colors"
            />
          </div>
          <div className="w-44">
            <label className="block text-[13px] font-medium text-text-secondary mb-2 font-['Inter']">Match Mode</label>
            <select
              value={replyMode}
              onChange={(e) => setReplyMode(e.target.value as ReplyMode)}
              className="w-full h-10 px-3.5 bg-bg-elevated border border-border rounded-lg text-sm text-white focus:outline-none focus:border-accent transition-colors"
            >
              {REPLY_MODES.map((m) => (
          ...
          <div className="w-44">
            <label className="block text-[13px] font-medium text-text-secondary mb-2 font-['Inter']">Reply Mode</label>
            <select
              value={responseMode}
              onChange={(e) => setResponseMode(e.target.value as ResponseMode)}
              className="w-full h-10 px-3.5 bg-bg-elevated border border-accent/30 rounded-lg text-sm text-accent focus:outline-none focus:border-accent transition-colors"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-bg-card border-2 border-[#2563EB40] rounded-[10px] overflow-hidden">
            ...
            <div className="max-h-[400px] overflow-auto">
              {allQuestions.map((q) => (
                <label
                  key={q.id}
                  className={`flex items-center gap-3 px-4 py-2.5 border-b border-border-subtle cursor-pointer transition-colors hover:bg-bg-elevated ${
                ...
                >
                  ...
                  <span className="text-[10px] text-text-muted font-['JetBrains_Mono'] px-1.5 py-0.5 rounded bg-bg-elevated">
                    {q.match_mode}
                  </span>
                </label>
              ))}
              ...
              {showNewQuestion && (
                <div className="flex items-center gap-2 px-4 py-3 border-t border-border-subtle">
                  <input
                    ...
                    placeholder="Keyword..."
                    className="flex-1 h-8 px-3 bg-bg-elevated border border-border rounded text-sm text-white placeholder:text-text-placeholder focus:outline-none focus:border-[#2563EB]"
                    autoFocus
                    onKeyDown={(e) => {
                  ...
                  <select
                    ...
                    className="h-8 px-2 bg-bg-elevated border border-border rounded text-xs text-white focus:outline-none"
                  >
                    {MATCH_MODES.map((m) => (
                      <option key={m.value} value={m.value}>
                        {m.label}
          ...
          <div className="bg-bg-card border-2 border-[#05966940] rounded-[10px] overflow-hidden">
            ...
            <div className="max-h-[400px] overflow-auto">
              {allAnswers.map((a) => (
                <label
                  key={a.id}
                  className={`flex items-start gap-3 px-4 py-2.5 border-b border-border-subtle cursor-pointer transition-colors hover:bg-bg-elevated ${
                ...
                >
                  ...
                  <span className="text-[10px] text-text-muted font-['JetBrains_Mono'] px-1.5 py-0.5 rounded bg-bg-elevated shrink-0">
                    {a.content_type}
                  </span>
                </label>
              ))}
              ...
              {showNewAnswer && (
                <div className="flex items-start gap-2 px-4 py-3 border-t border-border-subtle">
                  <textarea
                    ...
                    rows={3}
                    className="flex-1 px-3 py-1.5 bg-bg-elevated border border-border rounded text-sm text-white placeholder:text-text-placeholder focus:outline-none focus:border-green resize-none"
                    autoFocus
                  />
        ...
        <div className="flex items-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <select
              value={responseMode}
              onChange={(e) => setResponseMode(e.target.value as ResponseMode)}
              className="h-8 px-3 bg-bg-elevated border border-border rounded-md text-sm text-white focus:outline-none focus:border-accent transition-colors"
            >
              {RESPONSE_MODES.map((m) => (
                <option key={m.value} value={m.value}>
                  {m.label}
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              min={0}
              className="w-16 h-8 px-3 bg-bg-elevated border border-border rounded-md text-sm text-white font-['JetBrains_Mono'] focus:outline-none focus:border-accent transition-colors"
            />
          </div>
          <div className="flex items-center gap-2">
            <span className="text-text-muted">AI Limit:</span>
